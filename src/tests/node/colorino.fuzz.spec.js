...
const safeLog = (logger, ...args) => Result.fromThrowable(() => logger.log(...args), error => error)();
describe('Colorino - Node Environment - Fuzz Test', () => {
    ...
    describe('Random Object Inputs', () => {
        describe('with NO_COLOR=1', () => {
            ...
            test('should handle circular references gracefully', () => {
                const logger = createColorino(createTestPalette(), {});
                const circular1 = { name: 'root', self: null };
                circular1.self = circular1;
                const circular2 = { a: { b: null } };
    ...
    describe('Random Color Codes', () => {
        test('should handle invalid hex colors', () => {
            const result = Result.fromThrowable(() => createColorino(createTestPalette({ log: '#gggggg' })), error => error)();
        ...
        describe('with FORCE_COLOR=1', () => {
            ...
            test('should handle interleaved log levels', () => {
                ...
                for (let i = 0; i < 250; i++) {
                    ...
                    results.push(Result.fromThrowable(() => logger.info(`info ${i}`), error => error)());
                    results.push(Result.fromThrowable(() => logger.warn(`warn ${i}`), error => error)());
                    results.push(Result.fromThrowable(() => logger.error(`error ${i}`), error => error)());
                }
                const allSuccessful = results.every(r => r.isOk());
                expect(allSuccessful).toBe(true);
            });
...
//# sourceMappingURL=colorino.fuzz.spec.js.map