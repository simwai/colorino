...
describe('NodeColorSupportDetector', () => {
    describe('theme detection (sync OSC)', () => {
        test('uses overrideTheme when provided', ({ mockProcess }) => {
            const detector = new NodeColorSupportDetector(mockProcess, 'dark');
    ...
    describe('TTY detection', () => {
        test('should return NO_COLOR when not a TTY', ({ mockStdout, mockProcess, }) => {
            mockStdout.isTTY = false;
            const detector = new NodeColorSupportDetector(mockProcess);
            expect(detector.getColorLevel()).toBe(ColorLevel.NO_COLOR);
        });
        test('should detect ANSI when not a TTY but CLICOLOR_FORCE is set', ({ mockStdout, mockProcess, }) => {
            mockStdout.isTTY = false;
    ...
    describe('Edge Cases', () => {
        test('defaults to NO_COLOR with malformed FORCE_COLOR', ({ mockProcess, }) => {
...
//# sourceMappingURL=node-color-support-detector.spec.js.map