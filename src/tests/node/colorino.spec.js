...
describe('Colorino - Node Environment - Unit Test', () => {
    describe('Basic Logging Workflow (Critical Path)', () => {
        describe('with FORCE_COLOR=2', () => {
            ...
            test('outputs 256-color codes for green text', ({ stdoutSpy }) => {
                ...
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.GREEN_256}Hello, Colorino!${ANSI.RESET}\n`);
            });
        });
    ...
    describe('Color Format Conversion (Integration Point)', () => {
        describe('with FORCE_COLOR=3', () => {
            ...
            test('uses 24-bit truecolor codes', ({ stdoutSpy }) => {
                const logger = createColorino(createTestPalette({ log: '#ff5733' }), {});
                logger.log('test');
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.ORANGE_TRUE}test${ANSI.RESET}\n`);
            });
        });
        describe('with FORCE_COLOR=2', () => {
            test.scoped({ env: { FORCE_COLOR: '2' } });
            test('uses 256-color codes', ({ stdoutSpy }) => {
                const logger = createColorino(createTestPalette({ log: '#00ff00' }), {});
                logger.log('test');
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.GREEN_256}test${ANSI.RESET}\n`);
            });
        });
        describe('with FORCE_COLOR=1', () => {
            ...
            test('uses basic 16-color ANSI codes', ({ stdoutSpy }) => {
                const logger = createColorino(createTestPalette({ log: '#00ff00' }), {});
                logger.log('test');
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.GREEN_BASIC}test${ANSI.RESET}\n`);
            });
        });
    });
    describe('Multi-Argument Formatting', () => {
        describe('with NO_COLOR=1', () => {
            test.scoped({ env: { NO_COLOR: '1' } });
            test('concatenates multiple arguments with proper spacing', ({ stdoutSpy, logger, }) => {
    ...
    describe('Error and trace logging with cleaned stack', () => {
        test('removes Colorino internals from error stack traces', ({ stderrSpy, logger, }) => {
            ...
            expect(output, 'Should not leak MyColorino._out').not.toMatch(/MyColorino\._out/);
            expect(output, 'Should not leak _printCleanTrace').not.toMatch(/MyColorino\._printCleanTrace/);
            expect(output, 'Should not leak _cleanErrorStack').not.toMatch(/MyColorino\._cleanErrorStack/);
            expect(output, 'Should not leak dist path').not.toMatch(/\/colorino\/dist\//);
            expect(output, 'Should not leak compiled filenames').not.toMatch(/colorino\.[A-Za-z0-9]+\.mjs/);
        });
        test('removes Colorino internals from trace() stack traces', ({ stdoutSpy, logger, }) => {
            logger.trace('Trace');
            const output = stdoutSpy.getOutput();
            expect(output, 'Should not leak compiled filenames').not.toMatch(/colorino\.[A-Za-z0-9]+\.mjs/);
        });
    ...
    describe('Edge Cases', () => {
        ...
        describe('with NO_COLOR=1', () => {
            test.scoped({ env: { NO_COLOR: '1' } });
            test('handles arbitrary random strings without crashing', ({ stdoutSpy, logger, }) => {
    ...
    describe('All Standard Log Levels', () => {
        describe('with NO_COLOR=1', () => {
            test.scoped({ env: { NO_COLOR: '1' } });
            test('routes log/info/debug to stdout and warn/error to stderr', ({ stdoutSpy, stderrSpy, logger, }) => {
                logger.log('log');
                logger.info('info');
                logger.warn('warn');
                logger.error('error');
    ...
    describe('Concurrent Logger Instances', () => {
        describe('with FORCE_COLOR=1', () => {
            ...
            test('first logger outputs basic red color', ({ stdoutSpy }) => {
                ...
                logger1.log('red');
                expect(stdoutSpy.getOutput()).toContain(`${ANSI.RED_BASIC}red${ANSI.RESET}`);
            });
        });
        describe('with FORCE_COLOR=2', () => {
            test.scoped({ env: { FORCE_COLOR: '2' } });
            test('second logger outputs 256-color green', ({ stdoutSpy }) => {
                ...
                logger2.log('green');
                expect(stdoutSpy.getOutput()).toContain(`${ANSI.GREEN_256}green${ANSI.RESET}`);
            });
        });
    });
    describe('Object and Error Formatting with Newlines', () => {
        describe('with NO_COLOR=1', () => {
            ...
            test('adds newline before string after object', ({ stdoutSpy, logger, }) => {
                const obj = { id: 1 };
            ...
            test('handles consecutive strings without extra newlines', ({ stdoutSpy, logger, }) => {
                logger.log('First', 'Second', 'Third');
                expect(stdoutSpy.getOutput()).toBe('First Second Third\n');
            });
            test('handles complex mixed sequences correctly', ({ stdoutSpy, logger, }) => {
                const obj1 = { a: 1 };
                const obj2 = { b: 2 };
                const obj3 = { c: 3 };
            ...
            test('adds leading newline before Error objects', ({ stderrSpy, logger, }) => {
                const error = new Error('Test error');
                logger.error('Failed:', error);
                const output = stderrSpy.getOutput();
            ...
            test('adds newline before string after Error', ({ stderrSpy, logger, }) => {
                const error = new Error('Test error');
            ...
            test('truncates objects at max depth with [Object] placeholder', ({ stdoutSpy, logger, }) => {
                const deepObj = {
                ...
                expect(output, 'Should not leak deeply nested values').not.toContain('too deep');
            });
            test('handles circular references with [Circular] placeholder', ({ stdoutSpy, logger, }) => {
                const circular = { name: 'test' };
                circular.self = circular;
                logger.log(circular);
                const output = stdoutSpy.getOutput();
                expect(output, 'Should detect circular references').toContain('[Circular]');
            });
        });
        describe('with FORCE_COLOR=2', () => {
            test.scoped({ env: { FORCE_COLOR: '2' } });
            test('colorizes string arguments when followed by objects', ({ stdoutSpy, }) => {
                const logger = createColorino(createTestPalette({ log: '#00ff00' }), {});
                const obj = { id: 1 };
            ...
            test('colorizes all string arguments with the palette color', ({ stdoutSpy, }) => {
                const logger = createColorino(createTestPalette({ log: '#00ff00' }), {});
                logger.log('First', 'Second', 'Third');
                const output = stdoutSpy.getOutput();
                expect(output).toBe(`${ANSI.GREEN_256}First${ANSI.RESET} ` +
    ...
    describe('Colorize Helper', () => {
        describe('with FORCE_COLOR=3', () => {
            ...
            test('wraps text in 24-bit truecolor codes', ({ stdoutSpy }) => {
                ...
                logger.log(colored);
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.ORANGE_TRUE}OVERRIDE${ANSI.RESET}\n`);
            });
        });
        describe('with FORCE_COLOR=2', () => {
            test.scoped({ env: { FORCE_COLOR: '2' } });
            test('wraps text in 256-color codes', ({ stdoutSpy }) => {
                ...
                logger.log(colored);
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.GREEN_256}OVERRIDE${ANSI.RESET}\n`);
            });
        });
        describe('with FORCE_COLOR=1', () => {
            test.scoped({ env: { FORCE_COLOR: '1' } });
            test('wraps text in basic 16-color ANSI codes', ({ stdoutSpy }) => {
                ...
                const colored = logger.colorize('OVERRIDE', '#00ff00');
                logger.log(colored);
                expect(stdoutSpy.getOutput()).toBe(`${ANSI.GREEN_BASIC}OVERRIDE${ANSI.RESET}\n`);
            });
        });
        describe('with NO_COLOR=1', () => {
            test.scoped({ env: { NO_COLOR: '1' } });
        ...
        describe('Manual overrides with colorize', () => {
            describe('with FORCE_COLOR=2', () => {
                test.scoped({ env: { FORCE_COLOR: '2' } });
                test('allows mixing manual override (first arg) with themed text', ({ stdoutSpy, }) => {
                    const logger = createColorino(createTestPalette({ log: '#ffffff' }), {});
                ...
                test('allows mixing manual override (second arg) with themed text', ({ stdoutSpy, }) => {
                    const logger = createColorino(createTestPalette({ log: '#ffffff' }), {});
                    const override = logger.colorize('OVERRIDE', '#00ff00');
    ...
    describe('Gradient Helper', () => {
        describe('with FORCE_COLOR=3', () => {
            test.scoped({ env: { FORCE_COLOR: '3' } });
            test('applies truecolor gradient character-by-character', ({ stdoutSpy, }) => {
        ...
        describe('with FORCE_COLOR=2', () => {
            test.scoped({ env: { FORCE_COLOR: '2' } });
            test('applies 256-color gradient character-by-character', ({ stdoutSpy, }) => {
                const logger = createColorino(createTestPalette(), {});
        ...
        describe('with FORCE_COLOR=1', () => {
            test.scoped({ env: { FORCE_COLOR: '1' } });
            test('returns plain text and palette color applies via log()', ({ stdoutSpy, }) => {
                const logger = createColorino(createTestPalette({ log: '#ffffff' }), {});
                const gradient = logger.gradient('TEST', '#ff0000', '#0000ff');
                logger.log(gradient);
                const output = stdoutSpy.getOutput();
...
//# sourceMappingURL=colorino.spec.js.map