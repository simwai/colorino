...
describe('Colorino - Real Browser - Unit Test', () => {
    ...
    test('applies palette color to a single string', ({ mocks }) => {
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cBrowser console test', 'color:#00ff00');
    });
    test('applies palette color and keeps objects expandable', ({ mocks }) => {
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cObject data: \n%o', 'color:#ffffff', testObject);
    });
    ...
    test('allows per-call color override via colorize()', ({ mocks }) => {
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cOVERRIDE %cnormal', 'color:#ff5733', 'color:#ffffff');
    });
    test('does not persist manual colorize() overrides across calls', ({ mocks, }) => {
        const logger = createColorino(createTestPalette({ log: '#00ff00' }));
        const override = logger.colorize('OVERRIDE', '#ff5733');
        logger.log(override, 'first');
        expect(mocks.log).toHaveBeenNthCalledWith(1, '%cOVERRIDE %cfirst', 'color:#ff5733', 'color:#00ff00');
        logger.log('second');
        expect(mocks.log).toHaveBeenNthCalledWith(2, '%csecond', 'color:#00ff00');
    });
    test('combines overrides, palette strings and objects in order', ({ mocks, }) => {
        const logger = createColorino(createTestPalette({ log: '#00ff00' }));
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cpre %cFIRST %cmid \n%o %cSECOND %cpost', 'color:#00ff00', 'color:#ff0000', 'color:#00ff00', meta, 'color:#0000ff', 'color:#00ff00');
    });
    ...
    test('combines css(), colorize(), palette strings and objects in order', ({ mocks, }) => {
        const logger = createColorino(createTestPalette({ log: '#00ff00' }));
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cpre %cCSS %cmid \n%o %cOVR %cpost', 'color:#00ff00', 'color:orange', 'color:#00ff00', meta, 'color:#ff0000', 'color:#00ff00');
    });
    test('accepts raw CSS strings in css() helper', ({ mocks }) => {
        ...
        expect(mocks.log).toHaveBeenCalledWith('%cRaw %cplain', 'color:blue; text-decoration:underline;', 'color:#00ff00');
    });
    test('builds CSS strings from css() object style', ({ mocks }) => {
        ...
        logger.log(styled);
        expect(mocks.log).toHaveBeenCalledWith('%cStyled', 'color:red;background-color:black;font-weight:bold');
    });
