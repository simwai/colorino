...
describe('Color Utilities - Node & Browser Environemnt - Unit Test', () => {
  describe('hex to...', () => {
    ...
    it('toAnsi16: should convert hex to ANSI 16', () => {
      expect(rgbToAnsi16(hexToRgb('#ff0000'))).toBe(91)
    })
    it('toAnsi256: should convert hex to ANSI 256', () => {
      expect(rgbToAnsi256(hexToRgb('#ff0000'))).toBe(196)
    })
  })
  ...
  describe('rgb to...', () => {
    ...
    it('toAnsi256: should correctly convert grayscale values', () => {
      expect(rgbToAnsi256([0, 0, 0])).toBe(16) // black
      expect(rgbToAnsi256([255, 255, 255])).toBe(231) // white
      expect(rgbToAnsi256([128, 128, 128])).toBe(244) // gray
    })
  })

  describe('rgba to...', () => {
    it('toRgb: should convert RGBA to RGB', () => {
      expect(rgbaToRgb([255, 0, 0, 0.5])).toEqual([255, 0, 0])
    })
    it('toAnsi16: should convert RGBA to ANSI 16', () => {
      expect(rgbToAnsi16(rgbaToRgb([255, 0, 0, 0.5]))).toBe(91)
    })
    it('toAnsi256: should convert RGBA to ANSI 256', () => {
      expect(rgbToAnsi256(rgbaToRgb([255, 0, 0, 0.5]))).toBe(196)
    })
  })

  describe('hsl to...', () => {
    it('toAnsi16: should convert HSL to ANSI 16', () => {
      expect(rgbToAnsi16(hslToRgb([0, 100, 50]))).toBe(91) // red
    })
    it('toAnsi256: should convert HSL to ANSI 256', () => {
      expect(rgbToAnsi256(hslToRgb([0, 100, 50]))).toBe(196) // red
    })
  })

  describe('RGB Gradient Interpolation', () => {
    test('interpolates between two colors with 5 steps', () => {
      const result = hexGradient('#ff0000', '#0000ff', 5)
    ...
    test('handles single step by returning start color', () => {
      const result = hexGradient('#ff0000', '#0000ff', 1)
    ...
    test('handles two steps by returning start and end colors', () => {
      const result = hexGradient('#ff0000', '#00ff00', 2)
    ...
    test('interpolates grayscale gradients correctly', () => {
      const result = hexGradient('#000000', '#ffffff', 3)
    ...
    test('handles identical start and end colors', () => {
      const result = hexGradient('#ff5733', '#ff5733', 3)
