...
export class InputValidator {
  ...
  validateOptions(
  ...
  ): Result<boolean, ColorinoConfigError> {
    ...
    if (options.logLevel) {
      ...
      if (minimumLevel && !this.isValidLogLevel(minimumLevel)) {
        return err(
          new ColorinoConfigError('logLevel.min', 'invalid level', minimumLevel)
        )
      }
